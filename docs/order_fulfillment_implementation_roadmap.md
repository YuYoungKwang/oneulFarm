...
- 기존 주문 화면을 부분 수정하는 대신, 고객/운영자/배송사 화면을 역할별 신규 화면으로 구축하는 전략을 기본으로 한다.
...
완료 기준:
- 세 페이지가 별도 컴포넌트/라우트 기준으로 물리적으로 분리됨
- 각 역할에 맞는 버튼만 노출됨
...
### 3.2 프론트
- 고객/운영자/배송사 라우트 신규 분리
- 목록/상세/처리 패널 신규 컴포넌트 생성
- 상태 라벨 매핑 신규화
- 액션 버튼 조건부 렌더링
- 배송 추적 타임라인 UI
...
- 기존 주문 화면을 재활용하려 하면 구상태/신상태가 섞여 구조가 더 복잡해질 수 있음
